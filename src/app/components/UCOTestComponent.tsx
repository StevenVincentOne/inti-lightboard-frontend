...
export function UCOTestComponent() {
  const { 
    ...
    recentMessages,
    mode,
    refresh,    isUserConnected,    isTopicConnected,    isConversationConnected 
  } = useUCO();
  ...
  const getComponentStatus = (componentData: any) => {
    if (!componentData) return { status: (isUserConnected && isTopicConnected) ? "connected" : "disconnected", color: 'text-red-500', bg: 'bg-red-100' };

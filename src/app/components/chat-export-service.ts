// Chat Export Service - Sends local chat batches to PostgreSQL and Neo4j
// Handles 500-token exports and user-triggered "Save to Memory" actions

import LocalChatFileManager, { ExportBatch, ChatMessage } from './local-chat-file-manager';
...
export function useChatExport(userId: number) {
  const exportService = new ChatExportService(userId);

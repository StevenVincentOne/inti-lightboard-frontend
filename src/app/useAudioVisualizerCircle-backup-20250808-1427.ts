...
const drawCircleVisualization = (
  ...
  // Set transparency and bright yellow color
  canvasCtx.globalAlpha = 0.5;
  canvasCtx.strokeStyle = "#FFFF00"; // Direct bright yellow
...
// Function to draw Inti logo in the center
const drawIntiLogo = (
  canvas: HTMLCanvasElement,
  canvasCtx: CanvasRenderingContext2D,
  positioning: Positioning
  ...
  const logoSize = positioning.radius * 1.7; // Logo size relative to circle - 20% larger
  ...
  const cacheKey = 'inti-logo-cache';
  const canvasCache = canvas as HTMLCanvasElement & { [key: string]: HTMLImageElement };
  const cachedImg = canvasCache[cacheKey];
  
  if (cachedImg && cachedImg.complete) {
    // Draw cached image
    drawLogoImage(canvasCtx, cachedImg, centerX, centerY, logoSize, positioning);
  } else if (!cachedImg) {
    // Load image for the first time
    img.onload = () => {
      canvasCache[cacheKey] = img;
      drawLogoImage(canvasCtx, img, centerX, centerY, logoSize, positioning);
    };
    img.onerror = () => {
      // Fallback to text logo if image fails
      drawTextLogo(canvasCtx, centerX, centerY, positioning, "I");
    };
    img.src = '/inti-viz-logo.png';
...
// Helper function to draw the actual logo image (Inti)
const drawLogoImage = (
  canvasCtx: CanvasRenderingContext2D,
  ...
  // Create circular clipping path - larger clipping area
  canvasCtx.beginPath();
  canvasCtx.arc(centerX, centerY, positioning.radius * 0.7, 0, Math.PI * 2);
  canvasCtx.clip();
  ...
  // Draw the image
  canvasCtx.drawImage(img, drawX, drawY, drawWidth, drawHeight);
  
  canvasCtx.restore();
};

// Helper function to draw profile image (circular crop for user)
...
// Helper function to draw text fallback logo
const drawTextLogo = (
  canvasCtx: CanvasRenderingContext2D,
  centerX: number,
  centerY: number,
  ...
  const logoSize = positioning.radius * 0.6;
  
  // Create a circular clip to keep logo inside the visualizer bounds
  canvasCtx.beginPath();
  canvasCtx.arc(centerX, centerY, positioning.radius * 0.7, 0, Math.PI * 2);
  canvasCtx.clip();

  // Draw text logo with transparency
  canvasCtx.globalAlpha = 0.5;
  canvasCtx.fillStyle = "#FFFF00"; // Direct bright yellow
  canvasCtx.font = `bold ${logoSize}px var(--font-satoshi, 'Arial', sans-serif)`;
...
const drawPlayButton = (
  ...
  // Apply bright yellow with opacity
  canvasCtx.fillStyle = `rgba(255, 255, 0, ${opacity})`; // Bright yellow with opacity
  canvasCtx.fill();
};
...
export const useAudioVisualizerCircle = (
  ...
  const brightYellow = "#FFFF00"; // Direct bright yellow color
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // If not visible, clear canvas and stop drawing
      if (!isVisible) {
        canvasCtx.clearRect(0, 0, canvas.width, canvas.height);
        return;
      }
      ...
      // Schedule the next animation frame only if visible
      if (isVisible) {
      ...
      // Draw Inti logo for assistant visualizer (show immediately when visible)
      if (showLogo && isAssistant && isVisible) {
        drawIntiLogo(canvas, canvasCtx, currentPositioning);
      }

      // Draw user profile image for user visualizer (show immediately when visible)
      if (showLogo && !isAssistant && isVisible) {
        drawUserProfileImage(canvas, canvasCtx, currentPositioning, profileImageUrl);
      }
    };
    ...
    showLogo,
    isVisible,
    profileImageUrl, // Add profileImageUrl as a dependency
  ]);
